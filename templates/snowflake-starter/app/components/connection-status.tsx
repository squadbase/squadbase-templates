...
import { useSnowflakeStatus } from "@/hooks/use-snowflake-status";
...
export function ConnectionStatus() {
  ...
  const { data: databases, isLoading: isDatabasesLoading, error } = useSnowflakeDatabases();
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm md:text-base text-muted-foreground leading-relaxed">
                Snowflake connection is not configured. Run <span className="px-2 py-0.5 bg-destructive/10 rounded text-destructive font-mono text-sm font-semibold">Setup Snowflake</span> to create a connection and get started.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-green-700 dark:text-green-200">
                {databases?.length || 0} database{databases?.length !== 1 ? "s" : ""} accessible
              </p>
            </div>
          </div>
        </CardContent>
...
export function useIsSnowflakeSetupComplete() {
  const { data: status, isLoading: isStatusLoading } = useSnowflakeStatus();
  const { data: databases, isLoading: isDatabasesLoading, error } = useSnowflakeDatabases();
