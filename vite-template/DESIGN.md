...
`EChart` は `--chart-1` 〜 `--chart-5` の CSS 変数と Squadbase のライト／ダークテーマを自動解決する。`option.color` や軸ラインの色を手動で指定する必要はない（上書きしたい場合のみ指定）。テーマ切り替えにも追従する。
...
<Card>
  ...
  <CardContent>
    ...
    {/* スパークライン */}
    <EChart option={sparklineOption} height="40px" className="mt-2" />
  </CardContent>
</Card>
```
