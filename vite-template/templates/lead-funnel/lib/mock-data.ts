...
// Seeded RNG unique to this template (lead-funnel)
...
// Stage transition probability (Lead -> MQL -> SQL -> Appointment -> Opportunity)
...
// Per-channel volume weighting & cost profile
const SOURCE_PROFILE: Record<
  LeadSource,
  { weight: number; spendPerLead: number }
> = {
  "Organic Search": { weight: 0.26, spendPerLead: 18 },
  "Paid Search": { weight: 0.2, spendPerLead: 92 },
  "Paid Social": { weight: 0.17, spendPerLead: 64 },
  Referral: { weight: 0.1, spendPerLead: 12 },
  Webinar: { weight: 0.1, spendPerLead: 38 },
  Event: { weight: 0.07, spendPerLead: 145 },
  Outbound: { weight: 0.1, spendPerLead: 78 },
}
...
function stageForLead(): LeadStage {
  // Walk the funnel: each transition has STAGE_PASS_PROB; the lead's final
  // observed stage is the deepest stage it reached.
...
function generateFunnel(): FunnelStageStat[] {
  ...
  // Funnel must be in descending order — guaranteed by cumulative-from-top
  // logic but we sort defensively in case of zero-stage edge cases.
...
// ── IS member ranking by appointments ──
