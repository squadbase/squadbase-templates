...
export function ChannelAcquisition({ data }: ChannelAcquisitionProps) {
  // Sorted descending by lead count
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        return [
          ...
          `Spend: ${formatCurrency(c.spend, { short: true })}`,
          `CPL: $${c.cpl.toLocaleString("en-US")}`,
        ].join("<br/>")
      },
    },
    legend: { bottom: 0 },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-right tabular-nums text-muted-foreground">
            {row.original.mqls.toLocaleString("en-US")}
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-right tabular-nums">
            {row.original.appointments.toLocaleString("en-US")}
          </div>
        ),
      },
      {
  ...
  return (
    <DashboardCardPreset
      ...
      description="Lead volume per source with cost-per-lead (CPL) overlay. CPL is highlighted when it deviates more than 15% from the overall average."
    >
