...
export function OvertimeHeatmap({ data }: OvertimeHeatmapProps) {
  ...
  const option: EChartsOption = {
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        const memberName =
          memberOrder[memberOrder.length - 1 - p.value[1]]
        const week = weeks[p.value[0]]
        return [
          `<strong>${memberName}</strong>`,
          `Week: ${week}`,
          `Overtime: ${p.value[2]}h`,
        ].join("<br/>")
      },
    },
    grid: {
  ...
  return (
    <DashboardCardPreset
      title="Overtime Heatmap (Member × Week)"
      description="Detect recurring crunch on specific members and weeks"
    >
