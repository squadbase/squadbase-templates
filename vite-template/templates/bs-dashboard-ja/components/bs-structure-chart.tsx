...
export function BsStructureChart({ data }: BsStructureChartProps) {
  ...
  const option: EChartsOption = {
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        const side = p.dataIndex === 0 ? "資産" : "負債・純資産"
        const denom =
  ...
  return (
    <DashboardCardPreset
      title="BS構造図"
      description="最新月のスナップショット — 資産と負債・純資産の構成比"
    >
      <EChart option={option} height="320px" />
      <div className="mt-3 grid grid-cols-3 gap-3 border-t pt-3 text-xs">
        <div className="space-y-0.5">
          <div className="text-muted-foreground">総資産</div>
          <div className="font-semibold tabular-nums">
            {formatCurrency(data.totalAssets, { short: true })}
          </div>
        </div>
        <div className="space-y-0.5">
          <div className="text-muted-foreground">負債合計</div>
          <div className="font-semibold tabular-nums">
        ...
        <div className="space-y-0.5">
          <div className="text-muted-foreground">純資産</div>
          <div className="font-semibold tabular-nums">
