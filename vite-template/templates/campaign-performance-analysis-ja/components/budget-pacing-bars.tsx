...
export function BudgetPacingBars({ data }: BudgetPacingBarsProps) {
  return (
    <DashboardCardPreset
      title="予算消化進捗バー"
      description="月次サイクルにおける支出 vs 経過日数"
    >
          ...
          return (
            <div key={row.campaignId} className="space-y-1.5">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {row.daysElapsed}/{row.daysInPeriod} 日目 ・{" "}
                    {formatCurrency(row.spent, { short: true })} /{" "}
              ...
              <div className="flex justify-between text-xs text-muted-foreground tabular-nums">
                <span>消化 {formatPercent(row.pctSpent)}</span>
                <span>月内経過 {formatPercent(row.pctTimeElapsed)}</span>
              </div>
            </div>
          )
        })}
