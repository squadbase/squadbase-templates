...
function formatMetricValue(metric: string, value: number): string {
  ...
  if (metric === "sprint") return `${Math.round(value)} 回`
  if (metric === "minutes") return `${Math.round(value)} 分`
...
export function PositionHeatmap({ data }: PositionHeatmapProps) {
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        return [
          ...
          cell ? `値: ${formatMetricValue(metric, cell.value)}` : "",
          cell ? `スコア: ${Math.round(cell.normalized * 100)}` : "",
        ]
          .filter(Boolean)
          .join("<br/>")
      },
  ...
  return (
    <DashboardCardPreset
      title="ポジション別パフォーマンス"
      description="4 指標をポジションごとに正規化したヒートマップ"
    >
