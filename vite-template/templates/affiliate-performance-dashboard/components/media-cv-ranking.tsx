...
export function MediaCvRanking({ data }: MediaCvRankingProps) {
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        return [
          ...
          `ASP: ${row.asp}${row.isNew ? " (New)" : ""}`,
          `Conversions: ${formatNumber(row.conversions)}`,
          `Spend: ${formatCurrency(row.spend, { short: true })}`,
          `Revenue: ${formatCurrency(row.revenue, { short: true })}`,
  ...
  return (
    <DashboardCardPreset
      title="Media / ASP CV Ranking"
      description="Conversions by media-ASP pair. New media are highlighted."
    >
