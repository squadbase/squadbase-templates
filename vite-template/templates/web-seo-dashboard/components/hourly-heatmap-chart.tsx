...
export function HourlyHeatmapChart({ data }: HourlyHeatmapChartProps) {
  ...
  // ECharts heatmap Y axis increases from bottom to top.
  // We want Monday at the top, so reverse the Y values (Sun=0, ..., Mon=6)
  ...
  const option: EChartsOption = {
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        return [
          `<strong>${day} ${String(hour).padStart(2, "0")}:00</strong>`,
          `Pageviews: ${formatNumber(cell?.pageviews ?? 0)}`,
          `Unique Users: ${formatNumber(cell?.uniqueUsers ?? 0)}`,
        ].join("<br/>")
      },
    },
    grid: {
  ...
  return (
    <DashboardCardPreset
      title="Traffic by Day of Week & Hour"
      description="Peak and off-peak hours from the last 30 days of average pageviews"
    >
