...
export function AgeGenderHeatmap({ data }: AgeGenderHeatmapProps) {
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        return [
          ...
          `会員数: ${cell.memberCount.toLocaleString("ja-JP")}名 (${formatPercent(cell.share)})`,
          `平均 LTV: ¥${cell.avgLtv.toLocaleString("ja-JP")}`,
          `購入率: ${formatPercent(cell.purchaseRate)}`,
        ].join("<br/>")
      },
    },
    grid: {
  ...
  return (
    <DashboardCardPreset
      title="年代 × 性別 クロステーブル"
      description="年代と性別による会員数 — セル濃度が規模を表現"
    >
