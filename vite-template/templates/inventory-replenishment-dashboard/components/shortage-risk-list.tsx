...
export function ShortageRiskList({ data }: ShortageRiskListProps) {
  if (data.length === 0) {
    return (
      <DashboardCardPreset
        title="Shortage Risk SKUs"
        description="SKUs where stock days fall below the lead time"
      >
        <div className="flex items-center justify-center py-10 text-sm text-muted-foreground">
          No SKUs are currently at shortage risk.
        </div>
      </DashboardCardPreset>
    )
  }
  ...
  return (
    <DashboardCardPreset
      title="Shortage Risk SKUs"
      description="Ranked by buffer days (stock days minus lead time) — negative means stockout before delivery"
    >
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-1 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {item.sku} · {item.dailySalesAvg.toFixed(1)} units/day · lead{" "}
                    {item.leadTimeDays} d
                  </div>
                </div>
                <div className="flex items-center gap-3 text-right">
                  <div className="text-sm">
                    ...
                    >
                      buffer {item.bufferDays >= 0 ? "+" : ""}
                      {item.bufferDays.toFixed(1)} d
                    </div>
                  </div>
                  <Badge
                    variant="outline"
